...
class ProcessFighterDetail:
    ...
    def OneHotEncodeWin(self):
        ...
        self.fights = pd.concat([self.fights, pd.get_dummies(self.fights['win_by'], prefix='win_by')], axis=1)
    ...
    def CalculateFighterData(self):
        ...
        win_by_columns = ['win_by_Decision - Majority', 'win_by_Decision - Split', 'win_by_Decision - Unanimous',
                        'win_by_KO/TKO','win_by_Submission','win_by_TKO - Doctor\'s Stoppage']
    ...
    def ConvertHeightReachTo_cms(self):
        ...
        self.fighter_details['Height_cms'] = self.fighter_details['Height'].apply(self.convert_to_cms)
        self.fighter_details['Reach_cms']  = self.fighter_details['Reach'].apply(self.convert_to_cms)

    
    def ConvertWeightToPounds(self):
        self.fighter_details['Weight_lbs'] = self.fighter_details['Weight'].apply(lambda X: float(X.replace(' lbs.', '')) if X is not np.NaN else X)
    ...
    def MergeFrames(self):
        ...
        temp_blue_frame = self.temp_blue_frame.merge(self.fighter_details, left_on='hero_fighter', right_on='fighter_name', how='left')
        temp_blue_frame.set_index('index', inplace=True)

        temp_red_frame = self.temp_red_frame.merge(self.fighter_details, left_on='hero_fighter', right_on='fighter_name', how='left')
